...
unsigned int Hour_Entry = 0, Minute_Entry = 0,
             Second_Entry = 0;  // Flags to check if the
                                // CountDown_Hour,CountDown_Minute or
                                // CountDown_Second has been entered by the user
...
inline void CountDown_Mode_Changer() {
  ...
  if (GetAsyncKeyState(VK_DOWN)) {
    Main_Watch();
  }  // Changes over to Main Watch Mode if "Down Arrow Key" is pressed
}

void Scroll_Selector(int Number_of_Elements, unsigned int &Entry_Variable,
                     std::string Number_Suffix,
                     unsigned int &Entry_Checker)  // Function to scroll through
                                                   // a list of options
{
  int Selection_Index =
      0;  // Index to scroll through the elements during Selection
  ...
  while (Entry_Checker != 1) {
    ...
    if (GetAsyncKeyState(VK_RIGHT))  // Displays the next string if the "Right
    ...
    if (GetAsyncKeyState(VK_LEFT))  // Displays the previous string if the "Left
                                    // Arrow Key" is pressed
    {
      Selection_Index--;
    ...
    if (GetAsyncKeyState(VK_RETURN))  // Stores the currently displayed variable
...
void CountDown() {
  ...
  if ((Hour_Entry == 1) && (Minute_Entry == 1) && (Second_Entry == 1)) {
    while ((CountDown_Hours >= 0) || (CountDown_Minutes >= 0) ||
           (CountDown_Seconds > 0))  // Loop to display the CountDown
    {
      system("cls");
